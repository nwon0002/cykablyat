...
public class Attack extends SWAffordance implements SWActionInterface {
	...
	public void act(SWActor a) {
		SWEntityInterface target = this.getTarget(); // target to attack
		boolean targetIsActor = target instanceof SWActor; // target is alive
		...
		if (a.getHitpoints() <= 0){
            a.say("\t" + "The Attacker '" + a.getShortDescription() + "' has not enough hitpoints");
        }
		else if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {  //if the actor is carrying an item
				if (itemCarried.hasCapability(Capability.WEAPON)) { // attack with weapon
				    if (itemCarried.getHitpoints() <= 0){
                        a.say("\t" + "The weapon '" + itemCarried.getShortDescription() + "' has not enough hitpoints");
                    }else{
				        if(itemCarried.getSymbol().equals("†")){ // If the weapon is Lightsaber
                            double l = (double) a.getForce(); // force parse to double
                            double d = l/1000.0*itemCarried.getHitpoints(); //
                            int i = (int) d;
                            target.takeDamage(i);
							System.out.println(a.getShortDescription()+ " made "+ i + " points of damage ");
							itemCarried.takeDamage(1); // Lightsaber is indestructible
                    ...
                    if (itemCarried.getSymbol().equals("†") && (a.getForce()==0)) { // If lighsaber is taken by non-Force user
                        ...
                        a.say(a.getShortDescription() +": To use Lightsaber as a weapon, I need to have the Force");
                    }
					if (targetIsActor) {
