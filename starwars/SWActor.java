...
public abstract class SWActor extends Actor<SWActionInterface> implements SWEntityInterface {
	...
	/**NEW PART
	 * The amount of force <code>SWActor</code> has*/
	private int force;

    private SWAffordance attack;
	...
	public SWActor(Team team, int hitpoints, MessageRenderer m, SWWorld world) {
		...
		attack = new Attack(this, m);
		this.force = 0;
		//this.addAffordance(attack); this.removeAffordance(attack);
	}
	...
	@Override
	public void addCapability(Capability c) { capabilities.add(c); }

	@Override
	public boolean hasCapability(Capability c) { return capabilities.contains(c); }

    @Override
    public void removeCapability(Capability c) { capabilities.remove(c); }

    /**NEW PART
     * Returns the amount of force that <code>SWActor</code> has.
     * <p>
     * This method returns the integer value of the <code>force</code> of <code>SWActor</code>.
     * <p>
     * If this <code>SWActor</code> is not Jedi the amount of <code>force</code> is 0.
     *
     * @return 	the amount of the <code>force</code> of this <code>SWActor</code>
     * @see 	#force
     */
    public int getForce() { return this.force; }

    /**NEW PART
     * Sets the force of this <code>SWActor</code> to a new force <code>force</code>.
     * <p>
     * Useful when the instance of <code>SWActor</code> is created, to set initial amount of force
     *
     * @param 	i the force to set for this <code>SWActor</code>
     * @see 	#force
     */
    public void setForce(int i) { this.force = i;}

    /**NEW PART
     * Increments the <code>SWActor</code>'s <code>force</code> by <code>i</code>
     *
     * This method will add <code>i</code> to <code>SWActor</code>'s <code>force</code>.
     *
     * @param 	i the amount to increase the <code>force</code> by
     * @see 	#force
     */
	public void raiseForce(int i) { this.force = this.force + i;}
